...
static size_t write_cb(void *p, size_t sz, size_t n, void *u) {
    Buf *b = u;
     size_t r = sz * n;
    char *q = realloc(b->buf, b->len + r + 1);
     if (!q) return 0;
    memcpy(q + b->len, p, r); 
    b->len += r; 
    q[b->len] = 0; 
...
static int parse_user_array(const char *json, UserInfo *users, int max) {
    ...
    if (st && cJSON_IsString(st) && strcmp(st->valuestring, "OK") == 0 && res && cJSON_IsArray(res)) {
        int cnt = cJSON_GetArraySize(res);
         if (cnt > max) cnt = max;
        for (int i = 0; i < cnt; i++)
         {
             cJSON *u = cJSON_GetArrayItem(res, i); 
            if (u && fill_user(&users[added], u)) added++;
         }
    }
    cJSON_Delete(root); 
    return added;
...
static int add_label(char labels[MAX_PROBS][4], int count, const char *label) {
    ...
    cstr(labels[count], label, sizeof(labels[count]));
     return count + 1;
}
    ...
    for (int i = 0; i < count - 1; i++)
        for (int j = i + 1; j < count; j++)
            if (strcmp(labels[i], labels[j]) > 0) {
                char tmp[4];
                memcpy(tmp, labels[i], 4); 
                memcpy(labels[i], labels[j], 4);
                memcpy(labels[j], tmp, 4);
            }
}
...
int main(int argc, char **argv) {
    ...
    if (argc > 1) {
        ...
        { 
            cstr(handles[nhandles], argv[i], 64); 
    ...
    if (nusers != nhandles) {
        ...
        for (int hi = 0; hi < nhandles; hi++) {
            for (int i = 0; i < 3 && !s; i++) 
            { 
                s = cf_api("https://codeforces.com/api/user.info?handles=%s", handles[hi]); 
                if (!s) Sleep(500); 
            }
        ...
        if (ps) {
            ...
            if (probs && cJSON_IsArray(probs)) {
                ...
                for (int i = 0; i < sz; i++) {
                    ...
                    { 
                        pmap_ids[pmap_n] = cid; 
                        pmap_ratings[pmap_n] = jnum(p, "rating"); 
                        cstr(pmap_indices[pmap_n], idx, 4); 
                        pmap_n++;
                    }
                }
            }
    ...
    if (s) {
        ...
        if (cl_root) 
        { 
            cJSON *res = cJSON_GetObjectItemCaseSensitive(cl_root, "result"); 
            if (res && cJSON_IsArray(res)) { cl_arr = res; ncl = cJSON_GetArraySize(res); } 
        }
    }
    ...
    int nentries[MAX_HANDLES];
    SolvedProb *solved[MAX_HANDLES]; 
    int nsolved[MAX_HANDLES];
    ...
    for (int ui = 0; ui < nusers; ui++) {
        ...
        cJSON *rt = s ? cJSON_Parse(s) : NULL; free(s);
         s = NULL;
        cJSON *rt_arr = NULL; 
        int n = 0;
        if (rt) { cJSON *res = cJSON_GetObjectItemCaseSensitive(rt, "result"); 
            if (res && cJSON_IsArray(res)) { rt_arr = res; n = cJSON_GetArraySize(res); 
                if (n > MAX_CONTESTS) n = MAX_CONTESTS; } }

        s = cf_api("https://codeforces.com/api/user.status?handle=%s&from=1&count=%d", u->handle, MAX_SUBMISSIONS);
        cJSON *subj = s ? cJSON_Parse(s) : NULL; 
        free(s);
         s = NULL;
        cJSON *sub_arr = NULL; 
        int nsub = 0;
        if (subj) { cJSON *res = cJSON_GetObjectItemCaseSensitive(subj, "result"); 
            if (res && cJSON_IsArray(res)) { sub_arr = res; nsub = cJSON_GetArraySize(res); } }

        // 解析并筛选出 verdict 为 "OK(通过)" 的提交；
        // 同时构建：subs[]（用于判断某题是在赛内还是赛后过题）和 solved[]（用于统计各题难度分和图表去重）
        Sub *subs = malloc(sizeof(Sub) * (nsub > 0 ? nsub : 1)); 
        int sub_cnt = 0;
        ...
        for (int i = 0; i < n; i++) {
            cJSON *x = rt_arr ? cJSON_GetArrayItem(rt_arr, i) : NULL; 
            if (!x) continue;
    ...
    if (nmissing > 0) {
        fprintf(fp, "<p style='color:#ef4444;font-size:15px;margin-top:8px;font-weight:600'>未找到以下用户: ");
    ...
    for (int ui = 0; ui < nusers; ui++) {
        ...
        int n = nentries[ui];
        char fn[128]; 
        snprintf(fn, 128, "%s_report.html", u->handle);
        fp = fopen(fn, "w"); 
        if (!fp) continue;
        ...
        int ns = nsolved[ui];
        for (int i = 0; i < ns; i++) {
            char *us = strchr(su[i].problemId, '_'); 
            if (!us) continue;
            int cid = (int)strtol(su[i].problemId, NULL, 10); 
            const char *idx = us + 1;
        ...
        for (int i = 0; i < ns; i++) {
            ...
            for (int b = 0; b < nbins; b++) {
                if (su[i].rating >= ranges[b] && su[i].rating < ranges[b+1]) {
                    ba[b]++; if (su[i].time >= cy) by[b]++; 
                    if (su[i].time >= c180) b180[b]++; 
                    if (su[i].time >= cm) bm[b]++; 
                    break;
                }
            }
        }
